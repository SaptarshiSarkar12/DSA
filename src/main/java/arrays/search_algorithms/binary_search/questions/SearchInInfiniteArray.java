package arrays.search_algorithms.binary_search.questions;

/**
 * Question 5: Find position of an element in a sorted array of infinite numbers
 * <p>
 *     <a href="https://www.geeksforgeeks.org/dsa/find-position-element-sorted-array-infinite-numbers/">Question link</a>
 * </p>
 */
public class SearchInInfiniteArray {
    public static void main(String[] args) {
        int[] arr = {3, 5, 7, 9, 10, 90, 100, 130, 140, 160, 170};
        int target = 10;
        System.out.println("Index of " + target + " is " + findAns(arr, target));
    }

    static int findAns(int[] arr, int target) {
        // first find the range
        // first start with the chunk of size 2
        int start = 0;
        int end = 1;

        // Condition for the target to lie in that range
        while (target > arr[end]) {
            int temp = end + 1; // this is our new start
            // double the box value
            // end = previousEnd + sizeOfBox*2
            end = end + (end - start + 1) * 2;
            start = temp;
        }
        return binarySearch(arr, target, start, end);
    }

    static int binarySearch(int[] arr, int target, int start, int end) {
        while (start <= end) {
            int middle = start + (end - start) / 2;
            if (target < arr[middle]) {
                end = middle - 1;
            } else if (target > arr[middle]) {
                start = middle + 1;
            } else {
                return middle;
            }
        }
        return -1;
    }
}
